...
**Lifecycle:** `draft` → `published` → `archived`, via explicit action endpoints (`/publish`, `/archive`), never a generic status `PATCH`. A `draft` may be freely edited/hard-deleted; a `published` announcement may still be edited but never hard-deleted (archive instead); `archived` is fully immutable and terminal (no un-archive/republish).
...
**API:** self-service `GET /me/announcements`, `GET /me/announcements/{public_id}` (404 outside audience/draft/archived), `POST /me/announcements/{public_id}/acknowledge` (idempotent); management `GET/POST /announcements`, `GET/PUT/PATCH/DELETE /announcements/{public_id}`, `POST /announcements/{public_id}/publish`/`archive` (all `announcements.manage`).
