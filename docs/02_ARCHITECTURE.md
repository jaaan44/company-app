...
**Balance check occurs once, at submission.** For a paid Leave Type, a new submission is rejected if `total_days` would exceed `allocated_days - already-approved - already-pending` for that Staff/Leave Type/year — computed inside a `DB::transaction()` with `lockForUpdate()` on the relevant `leave_balances` row to prevent a concurrent-submission race — and **never re-checked at approval**, since the submission-time check already accounts for every other outstanding request. Unpaid Leave Types are never balance-checked. Self-service eligibility (active Staff status; sufficient balance) is skipped for Administrator-created requests (mirroring §22's Work Log precedent), but Leave Type activity and date-range/overlap validity are enforced identically for both creation paths — those are structural rules, not staff eligibility.
