...
- Lifecycle: `draft` → `published` (`POST .../publish`) → `archived` (`POST .../archive`), both explicit action endpoints, never a generic status `PATCH`. Archived is terminal (no un-archive/republish). A draft may be freely edited/hard-deleted; a published announcement may still be edited but never hard-deleted (archive instead); an archived announcement is fully immutable.
...
**Audience is atomic, never incremental.** `ValidatesAnnouncementAudience` (shared by Store/UpdateAnnouncementRequest) enforces that a `scoped` announcement always specifies at least one Department/Team, a `company_wide` one specifies neither, and — on update specifically — that touching `department_ids`/`team_ids` without also including `audience_type` is rejected outright, since the audience is always replaced as a whole (mirroring `ContactController`'s clear-then-set primary-contact transaction) rather than patched incrementally.
...
- `AnnouncementTest` (27 tests): creation (company-wide/scoped, validation, internal-ID rejection, creator accountability, Staff denial), scoped-audience coherence (at-least-one-target, company-wide-forbids-targets), editing (draft, published, archived-immutable, atomic-audience-update, audience-untouched-when-omitted), lifecycle (publish, duplicate-publish rejection, publish-from-archived rejection, server-controlled `published_at`, archive, duplicate-archive rejection, archive-from-draft rejection, no-unarchive), deletion vs. archival (draft deletable, published/archived not), management listing (drafts/archived included, `acknowledgements_count`, internal-ID rejection), relational integrity (Department/Team/Staff delete-protection).
...
- Full Phase 1–13 regression suite (467 pre-existing tests) verified passing unmodified in behavior.
...
- `php artisan test`: **528/528 passing, 1397 assertions** (full suite — Phases 1–14; 61 new tests net versus the pre-phase 467/1273).
...
9. `POST /api/v1/announcements/{public_id}/archive` — confirm `200`, `status: archived`; confirm it no longer appears in the Staff member's feed, and that further edit/publish/delete attempts on it all return `409`.
10. Attempt to delete the targeted Department or the acknowledging Staff member — confirm each is blocked with `409`.
