...
**Lifecycle.** `active`/`inactive`, never hard-deleted while referenced. Deactivating a Leave Type blocks it from being used in **new** Leave Requests (self-service and Administrator-created alike — see Employment Eligibility vs. Leave Entitlement/Balance Integrity below) while every existing Leave Request/Balance referencing it remains fully intact and readable (DEC-010, historical preservation). A Leave Type may be hard-deleted only while it has no referencing Leave Request or Leave Balance (`409` otherwise, backed by `restrictOnDelete()` — mirrors Department/Client's delete-protection pattern).
...
## Employment Eligibility vs. Leave Entitlement/Balance Integrity — What Administrator Creation Skips

Mirroring DEC-035's Work Log precedent, but drawing the eligibility/validity line differently for this domain — **corrected** from this document's original draft, which incorrectly extended the Work Log precedent to balance sufficiency (see DEC-036's Amendment):

- **Employment eligibility** (the requester's own Staff record must be `active`) is checked **only** for self-service creation. An **Administrator creating a request on a Staff member's behalf** (`POST /api/v1/leave-requests`) deliberately **skips this** — an Administrator is trusted to record/correct a historical request for a Staff member who has since become inactive or separated. This mirrors "Administrator-entered Work Logs skip this eligibility check entirely."
- **Leave entitlement/balance integrity is never bypassed, for anyone.** Sufficient paid-leave balance (`allocated_days - approved_days - pending_days >= requested_days`), Leave Type validity (the type must be `active`), and the **overlap**/**date-range** checks are all structural/entitlement rules, not staff eligibility, and are enforced **identically** for both creation paths — including balance sufficiency, which is *not* analogous to Work Log's eligibility check despite the surface-level parallel. There is no Administrator override or negative-balance concept anywhere in this module (DEC-036): an Administrator backfilling historical paid leave with no allocation on file must first set one via `POST /api/v1/staff/{public_id}/leave-balances`, then create the Leave Request. An inactive Leave Type is likewise closed to new business for everyone, Administrator included; only Administrator's own management endpoint may still reference an inactive type when correcting/deleting something that already exists (not creating a new Leave Request against it).
- Both the Form Request validation layer (`ValidatesLeaveBalanceAvailability`, a fast `422`) and the controller's transactional, `lockForUpdate()`-guarded re-check (`ChecksLeaveBalanceAvailability::assertSufficientBalance()`, a `409` on a genuine race) apply this same balance rule to self-service and Administrator creation via one shared implementation each — not two parallel, potentially-diverging copies.
...
- Administrator can view/approve/reject/cancel any request, create one on behalf of a Staff member (bypassing only that Staff member's active-employment-status requirement, never balance sufficiency), manage Leave Types, and set/update Leave Balance allocations.
- Overlapping `pending`/`approved` requests for the same Staff member are rejected; insufficient paid-leave balance is rejected at submission for **both** self-service and Administrator-created requests, with no override/negative-balance concept; unpaid Leave Types are never balance-checked.
