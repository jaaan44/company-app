...
- `published` → `archived`: `POST /api/v1/announcements/{public_id}/archive`. Only valid from `published` (`409` from `draft` — nothing to retire yet, delete it instead; `409` from `archived` — already archived). **This is the only lifecycle action a `published` Announcement ever accepts** — see Editing below for why a `published` Announcement's content itself is immutable.
- `archived` is terminal: no un-archive/republish endpoint. Correcting a mistake in already-published information means archiving the existing Announcement and creating a new corrected draft — the smallest coherent rule, mirroring Leave Management's "no edit; cancel and resubmit" precedent for a similarly terminal state. This workflow is deliberately **not** supported by revision/version history, a content-version table, edit history, or any form of acknowledgement migration/invalidation/reset — each new Announcement is simply a new, independent record.
...
Drafts exist because "hide future/unpublished content from ordinary employees" is a real, immediate requirement for a broadcast channel — you cannot safely compose a company-wide announcement in the same record employees can already query. A draft is visible only via the management endpoints (`announcements.manage`); it never appears in `/api/v1/me/announcements` or its detail endpoint, which only ever return `published` (non-archived) records. A draft may be freely edited (title/body/audience) and hard-deleted; publishing is the one-way door into employee visibility, and also the point at which the content becomes immutable (see Editing below).

## Editing — Draft-Only, Enforcing Historical Integrity

**Only a `draft` Announcement may be edited** (title/body/audience). Once `published`, an Announcement's title, body, and audience become **immutable content** — `PUT`/`PATCH` against a `published` Announcement returns `409`, the same as `archived`. This is a correction to this phase's original design (which allowed editing a `published` Announcement, on the reasoning that a low-risk typo/audience fix shouldn't require a full archive-and-republish): that design created a real historical-integrity gap. An employee's acknowledgement (see Acknowledgement below) records that they acknowledged *this specific Announcement*; if an Administrator could later rewrite its title/body/audience, the system would silently misrepresent what the employee actually acknowledged — directly contradicting this phase's own preserve-don't-mutate rationale for published company communications (DEC-009/DEC-010). Locking content at publication is the smallest rule that closes this gap without adding any of the mechanisms explicitly out of scope for V1 (revision/version history, a content-version table, edit history, acknowledgement migration, acknowledgement invalidation, or acknowledgement resets): **draft → published → archived, with editing permitted only while `draft`.**

Correcting a mistake in an already-published Announcement is a three-step workflow, not an edit: (1) archive the existing Announcement, (2) create a corrected draft, (3) publish the new Announcement. `status`/`published_at`/`published_by_user_id`/`created_by_user_id` are never touched by editing regardless — those are only ever changed by the lifecycle actions or creation itself.

**Resulting acknowledgement guarantee:** an acknowledgement permanently corresponds to the exact title/body/audience configuration that existed at the moment the Announcement was published — because that configuration can never subsequently change, there is nothing to migrate, invalidate, or reset when content is "corrected" (correcting it always means a different Announcement record entirely).

## Deletion vs. Archival

- A `draft` Announcement may be hard-deleted (`DELETE`) — it has never been broadcast, so there is no history to preserve. A *scoped* draft can genuinely have `announcement_departments`/`announcement_teams` pivot rows already (an Administrator may have set the audience before publishing) — deleting the draft is still unconditional and safe, because those rows are deleted right along with it via the `announcement_id` `cascadeOnDelete()` foreign key, and nothing outside the draft itself ever references them directly. Acknowledgement rows, by contrast, genuinely cannot exist for a draft, for a different reason: drafts are never employee-visible (`/me/announcements` only ever returns `published` records), and acknowledging requires the Announcement to be visible in the first place.
...
- An Administrator can create a draft Announcement, edit it (title/body/audience), and publish it; publishing sets `published_at`/`published_by_user_id` server-side.
...
- **Once `published`, an Announcement's title/body/audience are immutable** — `PUT`/`PATCH` returns `409` for both `published` and `archived`. Only `draft` accepts edits.
...
- An acknowledgement recorded against a published Announcement remains intact, unchanged, and attached to the same immutable content through archival — nothing ever rewrites or invalidates it.
...
See `docs/handoffs/V1_PHASE_14_HANDOFF.md` for what was actually implemented and tested; broad categories: Announcement CRUD/lifecycle (draft creation, draft title/body/audience editing, publish, duplicate-publish rejection, archive, duplicate-archive rejection, archived immutability, draft-only deletion); **content immutability after publication** (a published Announcement's title, body, and audience each independently rejected with `409` on edit attempt; an archived Announcement remains immutable; an acknowledged published Announcement cannot be edited); audience validation (company-wide/scoped coherence, at-least-one-target, atomic replacement, draft-only); employee visibility (company-wide, Department-scoped, Team-scoped, union across both, no-department/no-team staff, draft/archived hidden, unrelated audience hidden, current-membership-at-read-time behavior after a Staff move); acknowledgement (idempotency, privacy, 404 outside audience, survives archive unchanged, full draft→publish→acknowledge→archive retains its timestamp and record); authorization (Administrator-only management, Manager has no elevated authority, no-role/no-linked-staff/unauthenticated/suspended boundaries); relational integrity (Staff/Department/Team delete-protection); full Phase 1–13 regression.
