...
**Status:** PLANNING COMPLETE, REPOSITORY IMPLEMENTATION COMPLETE (corrected after review — see **Addendum 2**). See the **Addendum: Confirmed VPS Facts & Repository Implementation** and **Addendum 2: Review Corrections & Real Build Verification** at the end of this document. The remainder of this document is preserved exactly as originally written (per CLAUDE.md's "do not rewrite historical decisions unnecessarily") — it reflects this session's own planning and open questions at the time it was authored, before the product owner's VPS-discovery answers, implementation authorization, and subsequent review corrections below. Server-side execution (building/starting the stack, migrating the ad-hoc deployment, TLS/domain/firewall) remains **not done** — see `docs/DEPLOYMENT_STAGING.md` for the exact runbook and Addendum 2's "What remains for the VPS operator" list.
