...
**Status:** PLANNING COMPLETE, REPOSITORY IMPLEMENTATION COMPLETE — see the **Addendum: Confirmed VPS Facts & Repository Implementation** at the end of this document for the product owner's VPS discovery answers and what was actually built. The remainder of this document is preserved exactly as originally written (per CLAUDE.md's "do not rewrite historical decisions unnecessarily") — it reflects this session's own planning and open questions at the time it was authored, before the product owner's VPS-discovery answers and implementation authorization below. Server-side execution (building/starting the stack, migrating the ad-hoc deployment, TLS/domain/firewall) remains **not done** — see `docs/DEPLOYMENT_STAGING.md` for the exact runbook and the addendum's "What remains for the VPS operator" list.
...
| `docker/nginx/Dockerfile.staging` | Minimal staging Nginx image reusing the existing `docker/nginx/default.conf` vhost unchanged. Deliberately ships an *empty* `public/` directory rather than a copy of the application's own — verified (`grep -rln "@vite" apps/api/resources/views`) that `@vite` appears only in the unused default Laravel `welcome.blade.php`, so no compiled front-end asset is ever actually served; 100% of real traffic already reaches PHP-FPM via `try_files ... /index.php`, so this is a correctness-neutral simplification, not a shortcut. |
