...
Status: **Conceptual, with the Identity & Organization group's Roles/Permissions, Departments/Teams/Positions, and Staff (Phases 5–7), the Clients group (Phase 8), the Staff Operations group (Phase 9), and the Work Management group's Projects/Project Membership/Tasks/Work Logs (Phases 10–12) now implemented.** This document identifies likely entities, relationships, ownership concepts, and areas needing later design decisions. It intentionally does not specify every column — that happens per-phase, when each area is actually implemented.
